...
#cap = cv.VideoCapture('C:\\Users\\vasu0\\Desktop\\yolo obd\\hochschule.mp4')
whT = 320
...
#To find overlaping boxes ,based on confidance it will pick max confidence box (max over lap box)
    ...
    for i in indices: # Loop over indices to draw bounding boxes
        ...
        # print(x,y,w,h)
        cv.rectangle(img, (x, y), (x + w, y + h), (255, 100, 255), 2) # setting Initial and corner points of image
        # Putting text by using classNames and Ids to bbox
        cv.putText(img, f'{classNames[classIds[i]].upper()} {int(confs[i] * 100)}%',
                   (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 200), 2)
...
#while True:
    ...
    cv.putText(img,fps,(5,30),cv.FONT_HERSHEY_COMPLEX,1,(0,0,255),4)
    # Displaying output
    cv.imshow('Image', img)
    cv.waitKey(1)
